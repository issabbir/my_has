...
@section('content')
	<div class="row">
		<div class="col-12">
            ...
            <div class="card"><!----><!---->
                <div class="card-body">
                    <h4 class="card-title">House Replacement Allotment Letter List</h4>
                    <hr/>
			...
			<div class="card"><!----><!---->
				<div class="card-body"><h4 class="card-title">Allotment Letter Entry</h4><!---->
					...
					<form id="allotment-register" method="POST"
                        ...
                        @if(isset($data['allotmentLetter'][0]->allot_letter_no)))
                            action="{{ route('replaceAllotmentLetter.update', ['id' => $data['allotmentLetter'][0]->allot_letter_id]) }}">

                            <input name="_method" type="hidden" value="PUT">
                        @else
                            action="{{ route('replaceAllotmentLetter.store') }}">
                        @endif
                            {{ csrf_field() }}
                        <div class="row">
                            ...
                            <div class="col-md-12">
                                    ...
                                    <div class="row my-1">
                                        <div class="col-md-3">
                                            ...
                                            <input type="text" readonly value="" name="emp_allotted_building" class="form-control" id="emp_allotted_colony" />
                                        </div>
                                        <div class="col-md-3">
                                ...
                                @endif
                                    ...
                                    <div class="row my-2">
                                        ...
                                        <div class="d-flex justify-content-end col">
                                            ...
                                            @endif
                                            <input type="hidden" name="replace_app_id" id="replace_app_id" />
                                        </div>
                                    </div>
...
@endsection
    ...
    $(document).ready(function() {
        ...
        $('#allotmentLetterTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: APP_URL+"/replace-allotment-letter-list",
            columns: [
        ...
        $(document).on("click",".selectApplication",function(event) {
           let empCodeAndRepAppId = $(this).val().split('-##-');
           setEmployeeInformation(empCodeAndRepAppId[0],empCodeAndRepAppId[1]);
        });

        @if(isset($data['allotmentLetter'][0]->allot_letter_no))
            setEmployeeInformation('{!! $data['allotmentLetter'][0]->emp_code !!}','{!! $data['allotmentLetter'][0]->replace_app_id !!}');
        @endif
        ...
        function loadAdvertisementList(){
           // event.preventDefault(); event
                $.ajax({
                    type: "GET",
                    url: APP_URL+"/ha-replace-approved-datatable-list/",
        ...
        function setEmployeeInformation(employeeCode,repAppId)
        {
            //evt.preventDefault();
            $.ajax({
                type: "GET",
                url: APP_URL+'/ajax/employee-house-allocated-replaced-info/'+employeeCode,
                success: function (data) {
                    if(data.employeeInformation) {
                        ...
                        $('#emp_allotted_colony').val(data.employeeInformation.colony_name);
                        $('#replace_app_id').val(repAppId);
                    } else {
                        $('#emp_name').val('');
                        $('#emp_designation').val('');
                        $('#emp_department').val();
